...
// char target_directory[] = "/home/shinys/test";
...
static int do_filp_open__exit(struct file *f)
{
    ...
    if (fmode & (fmode_t)0x100000) { // FMODE_CREATED
        ...
        int nCompareVal=1;
        char tmp_path[256] = {0, };
        /*char msg_nokill[20] = "not killed!";
        char msg_kill[10] = "killed!";*/
        ...
        bpf_probe_read_kernel(tmp_path, sizeof(tmp_path), field->data);

        for (i=0; i<sizeof(target_directory) - 1; i++) {
            if (tmp_path[i] != target_directory[i]) {
                nCompareVal=0;
                //bpf_trace_printk(msg_nokill, sizeof(msg_nokill));
                break;
            }
        }
        
        if (nCompareVal == 1) {
            //bpf_trace_printk(msg_kill, sizeof(msg_kill));
            bpf_send_signal(9);
            goto out;
        }
